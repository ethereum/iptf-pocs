...
contract PetitionRegistryTest is Test {
    ...
    function test_PublishBatch_RevertsOnBatchSizeOutOfRange() public {
        (bytes32 petitionId, IPetitionRegistry.PetitionParams memory p) = _registerDefault();
        ...
        // Upper bound: batchSize > BATCH_SIZE_MAX must revert.
        pi.batchSize = uint32(registry.BATCH_SIZE_MAX()) + 1;
        vm.prank(relayer);
        vm.expectRevert(PetitionRegistry.BatchSizeOutOfRange.selector);
        registry.publishBatch(pi, hex"00", new bytes(48), new bytes(48 * 24));

        // Lower bound: batchSize == 0 must revert.
        pi.batchSize = 0;
        vm.prank(relayer);
        vm.expectRevert(PetitionRegistry.BatchSizeOutOfRange.selector);
        registry.publishBatch(pi, hex"00", new bytes(48), new bytes(48 * 24));
