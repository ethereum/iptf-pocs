...
contract PetitionRegistry is IPetitionRegistry {
    ...
    function _preflightBatch(BatchPublicInputs calldata pi) internal {
        ...
        if (pi.batchSize != BATCH_SIZE_MAX) revert BatchSizeOutOfRange();
    ...
    function _batchPublicInputs(BatchPublicInputs calldata pi) internal pure returns (bytes32[] memory out) {
        // 12 named PIs + 24 blsFields + 1 signerVkHash = 37 total.
        out = new bytes32[](12 + 24 + 1);
        out[0] = pi.petitionId;
        out[1] = pi.rRoot;
        out[2] = pi.predicateHash;
        out[3] = bytes32(uint256(pi.classIndex));
        out[4] = bytes32(uint256(pi.slot));
        out[5] = pi.priorRunningRoot;
        out[6] = pi.newRunningRoot;
        out[7] = pi.priorIdentityTagSetRoot;
        out[8] = pi.newIdentityTagSetRoot;
        out[9] = bytes32(uint256(pi.priorLeafCount));
        out[10] = bytes32(uint256(pi.newLeafCount));
        out[11] = pi.batchVersionedHash;
        for (uint256 k = 0; k < 24; k++) {
            out[12 + k] = pi.blsFields[k];
        }
        out[36] = pi.signerVkHash;
    }
