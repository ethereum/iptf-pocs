...
/// Maximum cardinality of an Organizer's `class_set`. Must match the
/// Resolution circuit's `CLASS_MAX = 16` and the Solidity registry's
/// `if (n > 16) revert ClassSetInvalid();` check at the same value.
pub const MAX_CLASS_SET_LEN: usize = 16;
