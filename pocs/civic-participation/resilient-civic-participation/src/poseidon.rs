...
/// BN254 t=5 Poseidon parameters (circom-style), cached on first use.
fn bn254_t5_params() -> &'static PoseidonParameters<Fr> {
    static PARAMS: OnceLock<PoseidonParameters<Fr>> = OnceLock::new();
    PARAMS.get_or_init(|| {
        light_poseidon::parameters::bn254_x5::get_poseidon_parameters::<Fr>(5)
            .expect("bn254 x5 t=5 params")
    })
}

/// Standard Poseidon-128 permutation over `[Fr; 5]` (BN254, t=5, R_F=8,
/// R_P=60, alpha=5). Produces the same end-state as the Noir
/// `poseidon::permute(consts::x5_5_config(), state)` instance the
/// circuits use; the equivalence is empirically pinned by the
/// `hash_4` cross-check test below.
pub fn permute_t5(state: &mut [Fr; 5]) {
    let params = bn254_t5_params();
    ...
    for round in 0..half {
        for i in 0..5 {
            state[i] += params.ark[round * 5 + i];
        }
        for s in state.iter_mut() {
            *s = s.pow(alpha);
        }
        apply_mds_t5(state, &params.mds);
    }
    ...
    for round in (half + params.partial_rounds)..total {
        for i in 0..5 {
            state[i] += params.ark[round * 5 + i];
        }
        for s in state.iter_mut() {
...
fn apply_mds_t5(state: &mut [Fr; 5], mds: &[Vec<Fr>]) {
    let mut next = [Fr::from(0u64); 5];
    for i in 0..5 {
        for j in 0..5 {
            next[i] += state[j] * mds[i][j];
        }
    }
    *state = next;
}
...
impl Poseidon1Sponge {
    /// Fresh sponge with `domain` seated in the capacity element.
...
mod tests {
    ...
    #[test]
    #[ignore = "prints values used to pin Noir cross-impl tests"]
    fn print_fsrt_test_vectors() {
        for seed in [1u64, 2, 7, 0x1234_5678u64] {
            let (v, s) = fsrt_prg_step(Fr::from(seed));
            println!(
                "seed={seed:#x}: v=0x{}, s=0x{}",
                hex::encode(fr_to_be_bytes(&v)),
                hex::encode(fr_to_be_bytes(&s)),
            );
        }
    }

    #[test]
    fn test_permute_t5_matches_light_poseidon_hash_4() {
        // hash_4(inputs) = state[0] after permute([0, inputs[0..4]]).
        // Pinning this equivalence against light-poseidon ensures the
        // Noir circuits (which use poseidon::permute on the same t=5
        // parameter set) and Rust produce identical sponge outputs.
        let trials: [[u64; 4]; 4] = [
            [0, 0, 0, 0],
            [1, 2, 3, 4],
            [0x1234_5678, 0xabcd_ef00, 0xdead_beef, 0xfeed_face],
        ...
        for trial in trials.iter() {
            ...
            let mut h = Poseidon::<Fr>::new_circom(4).expect("circom-4");
            let expected = h.hash(&inputs).expect("hash");

            let mut state = [Fr::from(0u64); 5];
            state[1..5].copy_from_slice(&inputs);
            permute_t5(&mut state);

            assert_eq!(
                state[0], expected,
                "permute_t5 disagrees with light_poseidon::hash_4 on {trial:?}"
            );
        }
    }
